...
class DHM_basic(nn.Module):
    ...
    def forward(self, state, inputs, STE, fn, pos_enc, recur_enc, max_step, mask, encoder_output=None):
        ...
        state = state.reshape(-1,T,D)
        inputs = inputs.reshape(-1,T,D)
        # init
        ...
        while ((halting_probability < self.threshold) & (n_updates < max_step)).byte().any():
            ...
            state = state.transpose(1,2) # BNTD

            state = state.view(-1,T,D)

            previous_state = ((state.cuda() * update_weights.unsqueeze(-1).cuda()) + (previous_state.cuda() * (1 - update_weights.unsqueeze(-1)).cuda()))

            step+=1
        return previous_state.view(B,N,T,D).transpose(1,2), (remainders,n_updates)
